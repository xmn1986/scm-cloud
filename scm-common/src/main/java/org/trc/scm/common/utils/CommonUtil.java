...
public class CommonUtil {
   ...
   * @throws
	...
	* @throws
	 ...
	 * @throws Exception
	 */
	public static Object getRequestParamFromHttpReqeust(HttpServletRequest request, Class<?> clasz, boolean isList) throws Exception{
		...
		if(jsonStr.length() == 0){
			throw new ParamValidException(CommonExceptionEnum.PARAM_CHECK_EXCEPTION, "外部HTTP请求参数不能为空");
		}
		String reqStr = jsonStr.toString();
		try{
			if(isList) {
				log.info("请求链接url:{}, 参数:{}",request.getRequestURL().toString(), reqStr);
				return $.json.toList(reqStr, clasz);
			}else {
				log.info("请求链接url:{}, 参数:{}",request.getRequestURL().toString(), reqStr);
				return $.json.toObject(reqStr, clasz);
			}
		}catch (Exception e){
			log.error("外部HTTP请求参数格式错误", e);
			throw new ParamValidException(CommonExceptionEnum.PARAM_CHECK_EXCEPTION, "外部HTTP请求参数格式错误"+e.getMessage());
		}
	}
